...
printHelp :: IO ()
printHelp = putStrLn "Usage: speed <distanceValue> <distanceUnit> <timeValue> <timeUnit>\n\
  ...
  case args of
    ...
    [distanceValueStr, distanceUnit, timeValueStr, timeUnit] -> do
      ...
      case (maybeDistanceValue, maybeTimeValue) of
        ([(distanceValue, "")], [(timeValue, "")]) -> do                        -- we pattern-match on [(distanceValue, "")] and [(timeValue, "")] to ensure the input is valid
